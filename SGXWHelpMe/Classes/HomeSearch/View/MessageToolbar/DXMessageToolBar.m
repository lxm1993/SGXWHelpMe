...
//    allButtonWidth += CGRectGetWidth(self.faceButton.frame) + kHorizontalPadding * 1.5;
    ...
    self.sendButton =  [[UIButton alloc] initWithFrame:CGRectMake(CGRectGetWidth(self.bounds) - kHorizontalPadding - kInputTextViewMinHeight-15, kVerticalPadding, kInputTextViewMinHeight+15, kInputTextViewMinHeight)];
    ...
    self.sendButton.layer.borderColor = [UIColor colorWithWhite:0.8f alpha:1.0f].CGColor;
    self.sendButton.layer.borderWidth = 0.65f;
...
//    self.inputTextView.contentMode = UIViewContentModeCenter;
    ...
    _inputTextView.placeHolder =@"我想要找";
    _previousTextViewContentHeight = [self getTextViewContentH:_inputTextView];
    
    width = width + kInputTextViewMinHeight + kHorizontalPadding*0.5 + 15;
    //录制
    ...
    [self.recordButton setTitle:kRecordToolBarUnTouch forState:UIControlStateNormal];
    [self.recordButton setTitle:kRecordToolBarTouch forState:UIControlStateHighlighted];
    ...
    if (!self.recordView) {
        self.recordView = [[DXRecordView alloc] initWithFrame:CGRectMake(0, 0, ([UIScreen mainScreen].bounds.size.width - 80), 200)];
    }
    ...
    switch (tag) {
        case 0://切换状态
        {
            ...
            [UIView animateWithDuration:0.2 delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
                ...
                self.sendButton.hidden = button.selected;
...
//    self.recordButton.highlighted = NO;
    if ([_recordView isKindOfClass:[DXRecordView class]]) {
        [(DXRecordView *)_recordView recordButtonTouchUpInside:nil andImageName:nil];
